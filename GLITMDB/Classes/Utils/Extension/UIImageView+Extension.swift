...
extension UIImageView {
    ...
    func loadPosterMovie(posterPath: String, placeholder: String = "image_placeholder") {
        kf.setImage(with: URL(string: "\(NetworkConfiguration.getAssetUrlPath(type: .poster))\(posterPath)"), placeholder: UIImage(named: placeholder),
                    options: [.transition(.fade(0.33)),
                              .memoryCacheExpiration(.seconds(60))])
    }
    
    func loadBackdropMovie(posterPath: String, placeholder: String = "image_placeholder") {
        kf.setImage(with: URL(string: "\(NetworkConfiguration.getAssetUrlPath(type: .backdrop))\(posterPath)"), placeholder: UIImage(named: placeholder),
                    options: [.transition(.fade(0.33)),
                              .memoryCacheExpiration(.seconds(60))])
    }
    
    func loadActorMovie(profilePath: String, placeholder: String = "image_placeholder") {
        kf.setImage(with: URL(string: "\(NetworkConfiguration.getAssetUrlPath(type: .avatar))\(profilePath)"), placeholder: UIImage(named: placeholder),
                    options: [.transition(.fade(0.33)),
                              .memoryCacheExpiration(.seconds(60))])
    }
}
